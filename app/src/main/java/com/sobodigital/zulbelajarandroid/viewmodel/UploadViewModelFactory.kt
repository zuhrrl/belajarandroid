...
class UploadViewModelFactory private constructor(private val localRepository: LocalRepository) :
    ViewModelProvider.NewInstanceFactory() {
    @Suppress("UNCHECKED_CAST")
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(UploadViewModel::class.java)) {
            return UploadViewModel(localRepository) as T
        }
    ...
    companion object {
        ...
        fun getInstance(context: Context): UploadViewModelFactory =
            instance ?: synchronized(this) {
                instance ?: UploadViewModelFactory(Injection.provideLocalRepository(context))
            }.also { instance = it }
    }
}